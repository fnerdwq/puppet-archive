        ...
        case $ensure {
          present: {
            ...
            exec {"download digest of archive ${name}":
              command => "curl ${basic_auth} ${redirect_arg} ${insecure_arg} ${proxy_arg} -L -s -o ${src_target}/${name}.${digest_type} ${digest_src}",
              path    => $exec_path,
  ...
  case $ensure {
    present: {
      ...
      exec {"download archive ${name} and check sum":
        command     => "curl ${basic_auth} -L -s ${redirect_arg} ${insecure_arg} ${proxy_arg} -o ${src_target}/${name} ${url}",
        path        => $exec_path,
